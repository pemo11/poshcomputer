...
def main(config: Config):
    ...
    while True:
        ...
        while True:
            print("\n[🤖] Thinking...")
            response, tool_calls = llm.query(messages, [pwsh.to_json_schema()])
            ...
            if tool_calls:
                for tc in tool_calls:
                    ...
                    if function_name != "exec_powershell_command" or "cmd" not in function_args:
                        ...
                        if confirm_execution(command):
                            tool_call_result = pwsh.exec_powershell_command(command)
                        else:
