...
class ExecOnConfirm:
    """
    A wrapper around the PowerShell tool that asks for user confirmation before executing any command.
    """

    def __init__(self, pwsh: PowerShell):
        self.pwsh = pwsh
    ...
    def exec_powershell_command(self, cmd: str) -> Dict[str, str]:
        """Execute a PowerShell command after confirming with the user."""
        if self._confirm_execution(cmd):
            return self.pwsh.exec_powershell_command(cmd)
...
def main(config: Config):
    ...
    agent = create_react_agent(
        model=llm,
        tools=[ExecOnConfirm(pwsh).exec_powershell_command],
    ...
    while True:
        ...
        user += f"\n Current working directory: `{pwsh.cwd}`"
        print("\n[🤖] Thinking...")
